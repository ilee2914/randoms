...
Map::Map(int id, Channel *channel) :
	...
	if (data) {
		...
		for (auto it : *mobs) {
			...
			int mobTime = it->mobTime;

			spawn_monster_init(life_id, x, y, foothold, mobTime, flip);
...
void Map::kill(Mob *mob) {
	...
	for (auto monster_id : data->mob_summons_) {
		spawn_monster(monster_id, mob->get_position_x(), mob->get_position_y(), mob->get_original_foothold(), mob_constants::kSpawnTypesSummonByMob, mob->get_stance() % 2 == 0, mob->getMobTime(), false, mob->get_object_id());
	}
...
void Map::spawn_monster(int mob_id, short x, short y, short foothold, signed char spawn_type, bool flip, int mobTime, bool mapMob,int from) {
