...
function action(mode, type, selection) {
    ...
    if (status == -1){
        start();
        return;
    } else if (mode != 1 || status == 7 || (actionx["1stJob"] && status == 4) || (cm.haveItem(4031008, 1) && status == 2) || (actionx["3thJobI"] && status == 1)){
