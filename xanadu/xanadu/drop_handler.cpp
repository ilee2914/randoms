...
void Player::handle_player_drop_mesos() {
	...
	map_->find_foothold(position_x_, position_y_);

	std::shared_ptr<Drop> drop(new Drop(map_->get_drop_object_id(), kDropPacketConstantsPickupTypesFreeForAll, position_x_, position_y_, dropped_mesos, 0, nullptr));
	map_->add_drop(drop);
	{
	...
	{
		PacketCreator packet;
		packet.ShowDrop(kDropPacketConstantsAnimationTypesShowDrop, drop, position_x_, position_y_);
		map_->send_packet(&packet);
	}
}
