...
void PacketCreator::InviteParty(Player *from)
{
	...
	write<signed char>(PartySendPacketActions::kInvite);
...
{
	write<short>(send_headers::kPARTY_OPERATION);
	write<signed char>(PartySendPacketActions::kCreate);
	write<int>(party_id);
	write<int>(kNoMap);
	write<int>(kNoMap);
	write<short>(0);
...
{
	write<short>(send_headers::kPARTY_OPERATION);
	write<signed char>(PartySendPacketActions::kRemoveOrLeave);
...
{
	write<short>(send_headers::kPARTY_OPERATION);
	write<signed char>(PartySendPacketActions::kJoin);
...
{
	write<short>(send_headers::kPARTY_OPERATION);
	write<signed char>(PartySendPacketActions::kChangeLeader);
...
{
	write<short>(send_headers::kPARTY_OPERATION);
	write<signed char>(PartySendPacketActions::kUpdate);
	write<int>(party->get_id());
	PartyInfo(party);
}
